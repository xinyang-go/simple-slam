...
/// \return 成功返回 CAMERA_STATUS_SUCCESS(0)。否则返回非0值的错误码, 请参考 CameraStatus.h 中错误码的定义。
/// \~english
/// \brief The ID of the current frame needs to be supported by the camera (supported by the avail range of network ports). This function returns an error code indicating that it is not supported.
/// \param [in] hCamera Camera handle.
...
/// \param [in] pCameraInfo camera's device descriptors information can be obtained by @link #CameraEnumerateDevice @endlink function.
...
/// \return 成功返回 CAMERA_STATUS_SUCCESS(0)。否则返回非0值的错误码, 请参考 CameraStatus.h 中错误码的定义。
/// \~english
/// \brief Clear all cached history_frames in the camera
/// \param [in] hCamera Camera handle.
/// \return Returns CAMERA_STATUS_SUCCESS(0) successfully. Otherwise, it returns a non-zero error code. Please refer to the definition of the error code in CameraStatus.h.
